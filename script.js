...
// SHOW UI TOUR STEP (WITH SIDE‑TOOLTIP LOGIC)
function showUITourStep() {
    ...
    // ⭐ SIDE‑PLACEMENT LOGIC FOR SIDEBAR BUTTONS
    if (el.closest('.sidebar')) {
        const sidebarIsRight = sidebar.classList.contains("sidebar-right");

        if (sidebarIsRight) {
            // Panel on RIGHT → Tooltip on LEFT
            tourTooltip.style.top = (rect.top + rect.height / 2 - tooltipHeight / 2) + "px";
            tourTooltip.style.left = (rect.left - tooltipWidth - 12) + "px";
        } else {
            // Panel on LEFT → Tooltip on RIGHT
            tourTooltip.style.top = (rect.top + rect.height / 2 - tooltipHeight / 2) + "px";
            tourTooltip.style.left = (rect.right + 12) + "px";
        }
    }

    // ⭐ NORMAL POSITIONING FOR NON‑SIDEBAR ELEMENTS
    else {
